...
const forumTopicInfo = asyncWrapper(async (req, res) => {
  ...
  const topicResponses = await ForumsTopicsModel.findById(
    { _id: topicID },
    "responses"
  );
  ...
  res.locals.topicResponses = topicResponses;

  res.status(StatusCodes.OK).render("./dashboard/public/single_forum", {
...
const createATopic = asyncWrapper(async (req, res) => {
  const { forumID, subject, topicText } = req.body;
  const { topicMediaFiles } = req.files;
    ...
    if (
      ...
      if (req.files || Object.keys(req.files).length > 0) {
        ...
        for (let i = 0; i < req.files.length; i++) {
          var topicMediaFile = req.files[files[i]];
      ...
      if (uploadOk == 1) {
        ...
        res.status(StatusCodes.OK).json({ topicInfo: topicCreated });
      }
    } else {
      res
        .status(StatusCodes.FORBIDDEN)
        .send(
          "You are not allowed to perform this action because you are not a member of this forum"
        );
    }
  }
});
...
const replyToATopic = asyncWrapper(async (req, res) => {
  const { topicID, taggedResponseID, replyText } = req.body;
  const { replyMediaFiles } = req.files;
  ...
  for (let i = 0; i < forumInfo.members.length; i++) {
    if (
      ...
      if (req.files || Object.keys(req.files).length > 0) {
        //Loop through each files
        for (let i = 0; i < req.files.length; i++) {
          var replyMediaFile = req.files[files[i]];

          allowedFiles = replyMediaFile.mimetype;
          fileSize = replyMediaFile.size;
          maxSize = 5000000;
          ...
          if (allowedFiles && allowedFiles.startsWith("image/")) {
            if (fileSize > maxSize) {
              ...
              const replyMediaFileName =
                uuidv4() + "_" + replyMediaFile.name.split(".")[0];

              cloudinary.uploader.upload(
                replyMediaFile.tempFilePath,
                {
                  resource_type: "image",
                  public_id: `forumTopicMediaFiles/${replyMediaFileName}`,
                },
                async function (err, logo) {
                  if (err) {
                    console.log(err);
                    ...
                    newReplyMedia = logo.secure_url;
                    uploadOk = 1;

                    replyMedia.push(newReplyMedia);

                    //Delete file from temp folder
                    fs.unlinkSync(replyMediaFile.tempFilePath);
                  }
                }
              );
            }
          ...
          if (allowedFiles && allowedFiles === "video/mp4") {
            //Check each file size
            if (replyMediaFile.size > maxSize) {
              res.send("File is too big");
              // res.json({ fileTooBig: true });
              uploadOk = 0;
            } else {
              ...
              await cloudinary.uploader.upload(
                ...
                async function (err, newFile) {
                  if (err) {
                    ...
                    newReplyMedia = newFile.secure_url;
                    uploadOk = 1;
      ...
      if (uploadOk == 1) {
        ...
        const replyPosted = await ForumsTopicsModel.findByIdAndUpdate(
          { _id: topicID },
        ...
        res.status(StatusCodes.OK).json({ replyInfo: replyPosted });
      }
    } else {
      res
        .status(StatusCodes.FORBIDDEN)
