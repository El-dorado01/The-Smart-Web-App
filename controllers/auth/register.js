...
const registerUser = asyncWrapper(async (req, res) => {
    ...
    const {
        html, errors
    } = mjml2html(mjmlData, mjmlOptions);
    ...
    async function sendMail() {
        try {
            ...
            // If the token is expired or about to expire (consider some buffer time), refresh it
            if (oAuth2Client.isTokenExpiring(accessToken)) {
